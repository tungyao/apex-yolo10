...
from ultralytics10.utils import (
    ASSETS,
    AUTOINSTALL,
    LINUX,
    LOGGER,
...
def parse_requirements(file_path=ROOT.parent / "requirements.txt", package=""):
    ...
    Args:
        ...
        package (str, optional): Python package to use instead of requirements.txt file, i.e. package='ultralytics10'.
    ...
    Example:
        ```python
        from ultralytics10.utils.checks import parse_requirements

        parse_requirements(package='ultralytics10')
        ```
    """
...
def check_version(
    ...
    if not current:  # if current is '' or None
        ...
        return True
    elif not current[0].isdigit():  # current is package name rather than version string, i.e. current='ultralytics10'
        try:
...
def check_latest_pypi_version(package_name="ultralytics10"):
    """
...
def check_pip_update_available():
    """
    Checks if a new version of the ultralytics10 package is available on PyPI.
        ...
        with contextlib.suppress(Exception):
            ...
            if check_version(__version__, f"<{latest}"):  # check if current version is < latest version
                LOGGER.info(
                    ...
                    f"Update with 'pip install -U ultralytics10'"
                )
                return True
    return False
...
def check_requirements(requirements=ROOT.parent / "requirements.txt", exclude=(), install=True, cmds=""):
    ...
    Example:
        ```python
        from ultralytics10.utils.checks import check_requirements

        # Check a requirements.txt file
        check_requirements('path/to/requirements.txt')

        # Check a single package
        check_requirements('ultralytics10>=8.0.0')

        # Check multiple packages
        check_requirements(['numpy', 'ultralytics10>=8.0.0'])
        ```
    """
...
def check_yolo(verbose=True, device=""):
    ...
    from ultralytics10.utils.torch_utils import select_device
...
def collect_system_info():
    ...
    from ultralytics10.utils import ENVIRONMENT, is_git_dir
    from ultralytics10.utils.torch_utils import get_cpu_info
    ...
    for r in parse_requirements(package="ultralytics10"):
        try:
...
def check_amp(model):
    ...
    Example:
        ...
        from ultralytics10.utils.checks import check_amp
