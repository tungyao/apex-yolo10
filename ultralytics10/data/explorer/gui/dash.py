...
from ultralytics10 import Explorer
from ultralytics10.utils import ROOT, SETTINGS
...
def _get_explorer():
    ...
    exp = Explorer(data=st.session_state.get("datasets"), model=st.session_state.get("model"))
    thread = Thread(
...
def init_explorer_form():
    ...
    with st.form(key="explorer_init_form"):
        ...
        with col1:
            st.selectbox("Select datasets", ds, key="datasets", index=ds.index("coco128.yaml"))
        with col2:
...
def query_form():
    """Sets up a form in Streamlit to initialize Explorer with datasets and model selection."""
...
def ai_query_form():
    """Sets up a Streamlit form for user input to initialize Explorer with datasets and model selection."""
